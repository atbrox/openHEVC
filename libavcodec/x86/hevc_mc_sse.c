    ...
    }else if(!(width & 7))
    {
        ...
        for (y = 0; y < height; y++) {
            for(x = 0;x < width; x+=2){
                ...
                r0 = _mm_srai_epi16(r0, 6);
                r0 = _mm_packus_epi16(r0, r0);
                *((int16_t *) (dst+x)) = _mm_extract_epi16(r0, 0);
            }
            dst += dststride;
            src += srcstride;
        }
        ...
        if(!(width & 15)){
            ...
            for (y = 0; y < height; y++) {
                for(x=0;x<width;x+=4){
                    ...
                    *((uint32_t *)(dst+x)) = _mm_cvtsi128_si32(x0);
            ...
            for (y = 0; y < height; y++) {
                for(x=0;x<width;x+=2){
                    ...
                    *((int16_t *) (dst+x)) = _mm_extract_epi16(x0, 0);
...
void ff_hevc_put_hevc_epel_hv_8_8_sse(int16_t *_dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height + EPEL_EXTRA; y++) {
        for (x = 0; x < width; x += 8) {
            ...
            x1 = _mm_lddqu_si128((__m128i *) &src[x]);
            x2 = _mm_shuffle_epi8(x1, bshuffle1);
            x1 = _mm_lddqu_si128((__m128i *) &src[x + 4]);
...
void ff_hevc_put_hevc_qpel_h_1_10_sse(int16_t *dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height; y ++) {
        for (x = 0; x < width; x += 2){
            ...
            *((uint32_t *)(dst+x)) = _mm_cvtsi128_si32(x1);
...
void ff_hevc_put_hevc_qpel_h8_1_v_1_sse(int16_t *dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height + ff_hevc_qpel_extra[1]; y++) {
        for (x = 0; x < width; x += 8) {
            /* load data in register     */
            __m128i m0, m1, m2, m3, m4, m5, m6, m7;
            m0 = _mm_lddqu_si128((__m128i *) &src[x - 3]);
            m1 = _mm_lddqu_si128((__m128i *) &src[x - 2]);
            m2 = _mm_lddqu_si128((__m128i *) &src[x - 1]);
            m3 = _mm_lddqu_si128((__m128i *) &src[x]);
            m4 = _mm_lddqu_si128((__m128i *) &src[x + 1]);
            m5 = _mm_lddqu_si128((__m128i *) &src[x + 2]);
            m6 = _mm_lddqu_si128((__m128i *) &src[x + 3]);
            m7 = _mm_lddqu_si128((__m128i *) &src[x + 4]);
...
void ff_hevc_put_hevc_qpel_h8_3_v_1_sse(int16_t *dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height + ff_hevc_qpel_extra[1]; y++) {
        for (x = 0; x < width; x +=8) {
            ...
            // load data in register
            x7 = _mm_loadu_si128((__m128i *) &src[x-2]);
            x1 = _mm_unpacklo_epi64(x7, x7);
            x7 = _mm_loadu_si128((__m128i *) &src[x]);
            x3= _mm_unpacklo_epi64(x7, x7);
            x7 = _mm_loadu_si128((__m128i *) &src[x+2]);
            x4= _mm_unpacklo_epi64(x7, x7);
            x7 = _mm_loadu_si128((__m128i *) &src[x+4]);
...
void ff_hevc_put_hevc_qpel_h8_3_v_2_sse(int16_t *dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height + ff_hevc_qpel_extra[2]; y++) {
        for (x = 0; x < width; x +=8) {
            ...
            // load data in register
            x8 = _mm_loadu_si128((__m128i *) &src[x-2]);
            x1 = _mm_unpacklo_epi64(x8, x8);
            x8 = _mm_loadu_si128((__m128i *) &src[x]);
...
void ff_hevc_put_hevc_qpel_h4_3_v_3_sse(int16_t *dst, ptrdiff_t dststride,
    ...
    for (y = 0; y < height + ff_hevc_qpel_extra[3]; y ++) {
        for (x = 0; x < width; x +=4) {
            ...
            x3 = _mm_loadu_si128((__m128i *) &src[x-2]);
            x1 = _mm_unpacklo_epi64(x3, x3);
            x3 = _mm_loadu_si128((__m128i *) &src[x]);
            x3= _mm_unpacklo_epi64(x3, x3);
